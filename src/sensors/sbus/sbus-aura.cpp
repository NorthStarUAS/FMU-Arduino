...
void sbus_aura_t::parse() {
    ...
    Serial.print(millis()); Serial.print(" ail: "); Serial.print(pwm_val[3]); Serial.print(" "); Serial.println(norm_val[3], 3);
    // inceptors_node.setUInt("debug", pwm_val[3]);

#if 0
    Serial.print(" ");
    Serial.print(raw_val[0]);
    Serial.print(" ");
    Serial.print(raw_val[1]);
    Serial.print(" ");
    Serial.print(raw_val[2]);
    Serial.print(" ");
    Serial.print(raw_val[3]);
...
void sbus_aura_t::init() {
    Serial2.begin(100000,SERIAL_8E2_RXINV_TXINV); // newer teensies should use SERIAL_8E2_RXINV_TXINV
    ...
    if  ( state == 2 ) {
        ...
        if ( Serial2.available() > 0 ) {
            ...
            if ( input == SBUS_FOOTER_VALUE ) {
                parse();
                state = 0;
                // Serial.print("sbus:" ); Serial.println(millis());
            } else {
